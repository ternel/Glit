...
class Branch {
    ...
    public function getTip($noCache = false) {
        ...
        $subpath = sprintf('refs' . DS . 'heads' . DS . '%s', $this->branchName);
        $path    = $this->repository->getInternalPath()->buildSubPath($subpath);

        if (file_exists($path)) {
            $sha            = substr(file_get_contents($path), 0, 40);
            $this->tipCache = new Commit($this->repository, new SHA($sha));
            return $this->tipCache;
        }

        $path = sprintf('%s/packed-refs', $this->repository->getDir());
        if (file_exists($path)) {
            $head = NULL;
            $f    = fopen($path, 'rb');
            flock($f, LOCK_SH);
            ...
            fclose($f);
            if ($head !== NULL) {
                $this->tipCache = new Commit($this->repository, new SHA($head));
                return $this->tipCache;
            }
        }
